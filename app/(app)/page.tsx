  ...
  if (!loadingCondominiums && condominiums.length === 0) {
    return (
      <main className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        <h1 className="text-4xl font-black text-white">Nenhum condomínio cadastrado</h1>

        <p className="mt-4 max-w-md text-slate-400">
          Cadastre o primeiro condomínio para começar a monitorar blocos e reservatórios.
        </p>

        <Link
          href="/condominios"
          className="mt-8 rounded-2xl bg-brand-gold px-6 py-3 font-semibold text-brand-deep transition hover:bg-[#e0c15c]"
        >
    ...
    <main className="space-y-8">
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-[1.7rem] border border-white/10 bg-brand-petrol p-6">
          ...
          <h3 className="mt-4 text-5xl font-black">{condominiums.length}</h3>
        </div>
        ...
        <div className="rounded-[1.7rem] border border-white/10 bg-brand-petrol p-6">
          ...
          <h3 className="mt-4 text-5xl font-black">
            {sensorsOnline}/<span className="text-emerald-400">{blocks.length}</span>
          </h3>
        </div>
